...
function initialize(useBigInt) {
  if (useBigInt) return { bigInt: (x) => BigInt(x), ZERO: 0n, ONE: 1n, TWO: 2n, MODULO: undefined };
  else return { bigInt: (x) => x, ZERO: 0, ONE: 1, TWO: 2, MODULO: $MODULO };
}
...
class ColumnState {
  constructor(left, value, count, blocks, reachedTop) {
    ...
    this.key = this.value * FOUR + (this.blocks ? TWO : ZERO) + (this.reachedTop ? ONE : ZERO);
  }

  get isValid() {
    if (!this.reachedTop) return false;
  ...
  nextState(value, maxHeight) {
    ...
    if (newBlocks < ZERO) newBlocks = ZERO;
...
class ColumnEngine {
  ...
  forEachStates(callback) {
    if (this.useLeft) {
      this.states.forEach((leftMap) => {
        if (leftMap.size === undefined) callback(leftMap);
        else leftMap.forEach((state) => callback(state));
      });
    } else {
  ...
  addNewState(state) {
    ...
    if (o) {
      o.count = MODULO ? (o.count + state.count) % MODULO : o.count + state.count;
    } else {
  ...
  get total() {
    ...
    this.forEachStates((s) => {
      total = MODULO ? (total + s.total) % MODULO : total + s.total;
    });
  ...
  quickMerge(trace) {
    const tracer = new Tracer(trace, 'merging');
      ...
      this.forEachStates((rightState) => {
        if (leftState.reachedTop || rightState.reachedTop) {
          ...
          if (!blocks) {
            ...
            total = MODULO ? (total + count) % MODULO : total + count;
          }
        }
      });
    });
  ...
  merge() {
    const lefts = [];
    this.forEachStates((state) => {
      if (lefts[state.left] === undefined) lefts[state.left] = [state];
      else lefts[state.left].push(state);
    });

    this.forEachStates((leftState) => {
      const rightStates = lefts[leftState.value];
      if (rightStates === undefined) throw 'ERROR';
      ...
      for (const rightState of rightStates) {
        ...
        const newState = new ColumnState(
          leftState.left,
          rightState.value,
          count,
          blocks,
...
function F3(width, height, trace) {
  ...
  function findPolynomial(start) {
    const tracer = new Tracer(trace, 'populating values');
...
assert.equal(
  timeLogger.wrap('13x10', (_) => F3(13, 10)),
  50584533
);
assert.equal(
  timeLogger.wrap('10x13', (_) => F3(10, 13)),
  959702255
);
