...
import { createMatrix } from '../build/debug.js';

// const { matrixSmall: Matrix } = require('@dn0rmand/project-euler-tools');

const MODULO = 1e9;

class State {
    constructor(value) {
        if (typeof value === 'number') this.init(value);
        else this.clone(value);
    }
...
function nCp(n, p) {
    const key = n * 20 + p;
    if ($nCp[key] !== undefined) return $nCp[key];
    ...
    function processState(rowIndex, data, missedOne) {
        ...
        for (let one = count; one >= 0; one--) {
            for (let two = count - one; two >= 0; two--) {
                ...
                if (newData[0] === 0 && (missedOne || newData[1] === 0)) continue;
...
function F1(m, n) {
    ...
    const add = (s) => {
        ...
        if (o) o.count = (o.count + s.count) % MODULO;
        else newStates.set(k, s);
    };

    const dump = (_) => {
        const st = [...newStates.values()].sort((a, b) => {
            let v = (b.missed ? 0 : 1) - (a.missed ? 0 : 1);
            if (v === 0) v = b.data[0] - a.data[0];
            if (v === 0) v = b.data[1] - a.data[1];
            if (v === 0) v = b.data[2] - a.data[2];
        ...
        for (const state of states.values()) {
            ...
            for (let one = count; one >= 0; one--) {
                for (let two = count - one; two >= 0; two--) {
                    ...
                    if (newState.data[0] === 0 && (newState.missed || newState.data[1] === 0)) continue;
    ...
    for (const s of states.values()) {
        if (s.data[0] === 2 * m) total = (total + s.count) % MODULO;
    }
